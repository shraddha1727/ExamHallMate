...
import { Home, Users, Grid, BookOpen, Layers, UserCheck, Calendar, User, LogOut, Menu, X } from 'lucide-react';
  ...
  return (
    <>
      ...
      <button
        ...
        className="md:hidden fixed top-4 right-4 z-[110] p-2 bg-slate-900 text-white rounded-lg shadow-lg border border-slate-700"
      >
      ...
      <aside className={`w-64 h-screen fixed left-0 top-0 flex flex-col z-[100] bg-gradient-to-b from-slate-900 to-slate-950 text-white border-r border-slate-800 no-print shadow-2xl transition-transform duration-300 md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {/* Brand Header */}
